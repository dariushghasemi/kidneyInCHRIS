...
#------------#
# Add rsID of SNPs with missing identifier
add_mis_rsid <- function(df) {
  df %>% mutate(
...
#------------#
# extract alleles from CHR:POS_RA_EA in CHRIS
marker_to_allele <- function(df) {
  df %>% mutate(
    SNPid           = str_c("chr", CHR, ":", POS38),  #CHR:POS
...
#effect direction when CKDGen EA is discordant to CHRIS
...
#------------#
# align association direction to both studies have negative effect estimates
aligning_direction <- function(df) {
  df %>% mutate(
...
#------------#
# filling missing EA/OA, EAF, beta, and SE properly 
add_mis_allele <- function(df) {
  df %>% mutate(
    SNPid = str_remove(SNPid, "chr"),
    EA_OA = case_when(
...
#------------#
# add SNPs MAF and effect size ratio in CHRIS and CKDGen
add_studies_ratio <- function(df){
  df %>% mutate(
...
#------------#
# 162 replicated snps in trans-ethnic CKDGen GWAS
#repSNPs_MA_old <- 
repSNPs_old %>%
  rename(EA_CKDGen_disc  = EA_CKDGen,
         ...
         Locus           = Closest.Gene,
         n_CKDGen_MultiA = n_total_sum_CKDGen) %>%
  add_mis_rsid() %>% 
  marker_to_allele() %>%
  aligning_allele() %>%
  aligning_direction() %>%
  ...
  select(Locus, RSID, SNPid, EA_OA,
         EAF_CKDGen, Beta_CKDGen_ald, SE_CKDGen, Pvalue_CKDGen, n_CKDGen_MultiA,
         EAF_CHRIS,  Beta_CHRIS_ald,  SE_CHRIS,  Pvalue_CHRIS,  Pvalue_CHRIS_1s) %>%
  ggplot(aes(x = Beta_CKDGen_ald, y = Beta_CHRIS_ald, color = Locus)) + 
  geom_point(alpha = 0.9, size = 3.5)
  #write.csv("11-Dec-23_Suppl. Table 3_163 replicated SNPs in CHRIS_MultiA.csv", row.names = F, quote = F)
...
repSNPs %>%
  add_mis_allele() %>%
  select(Locus, RSID, SNPid, EA_OA,
         EAF_CKDGen, Beta_CKDGen_ald, SE_CKDGen, Pvalue_CKDGen, n_total_sum_CKDGen,
         EAF_CHRIS,  Beta_CHRIS_ald,  SE_CHRIS,  Pvalue_CHRIS,  Pvalue_CHRIS_1s, NS_CHRIS) %>%
  write.csv("11-Dec-23_Suppl. Table 3_163 replicated SNPs in CHRIS_EUA.csv", row.names = F, quote = F)
